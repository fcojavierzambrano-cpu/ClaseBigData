...
// pregunta 11
    a: scala> df.filter($"Close"<600).show()
    b: 
    c: scala> df.select(corr("High", "Volume")).show()
+--------------------+
|  corr(High, Volume)|
+--------------------+
|-0.20960233287942157|
+--------------------+
    ...
    d:
    scala> dfmins.select($"Year", $"max(High)").show()
+----+------------------+
|Year|         max(High)|
+----+------------------+
|2015|        716.159996|
...
e:
scala> val dfavgs = df3.groupBy("Month").mean()
val dfavgs: org.apache.spark.sql.DataFrame = [Month: int, avg(Open): double ... 6 more fields]
